...
class QuoteListSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Quote
        fields = ('id', 'quote_no', 'status', 'expiry_date', 
                  'payable_amount', 'price_breakdown', 'remarks', 'created_at',
...
class QuoteItemSerializer(serializers.Serializer):
    ...
    selling_price = serializers.DecimalField(min_value=1000, max_digits=25, decimal_places=4, required=True)
...
class QuoteItemDetailSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = QuoteItem
        fields = ('product_code', 'supplier_code', 'order_quantity', 'status', 'selling_price', 'shipping_price')
...
class OrderDetailSerializer(serializers.Serializer):
    ...
    class Meta:
        fields = ('product_code', 'supplier_code', 'supplier_city', 'qty', 'weight', 'price')
...
class QuotePriceSerializer(serializers.Serializer):
    ...
    class Meta:
        fields = ('subtotal_price', 'shipping_price', 'total_payable', 'orders', 'shipping_details')
...
class QuoteDetailSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = ('id', 'quote_no', 'status', 'expiry_date', 
                  'payable_amount', 'price_breakdown', 'remarks', 'created_at',
                  'customer_name', 'customer_code', 'items')
        read_only_fields = ('id', 'created_at',)



class CreatePOSerializer(serializers.Serializer):
    order_date = serializers.DateField(input_formats=[DATE_FORMAT, 'iso-8601'], required=False)
    orders = serializers.ListField(child=QuoteItemSerializer(), min_length=1, max_length=20)
    ...
    def validate(self, attrs):
        ...
        for item in attrs['orders']:
            if item['product_code'] not in order_item:
                ...
                raise serializers.ValidationError("Duplicate product code found in the order")
