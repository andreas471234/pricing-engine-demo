import datetime
from api.serializers.quote_serializers import CreatePOSerializer, QuoteDetailSerializer, QuoteListSerializer, QuotePriceSerializer
...
class PricingEngine:
    ...
    def calculate_price(self):
        ...
        dynamic_margin_percentage = Decimal((self.market_price - self.avg_past_sell_price) / self.market_price, ) * Decimal(0.2)
...
class QuoteManager(BaseManager):
    ...
    def get_product_price(self, item):
        ...
        if not supp_prod_list.exists():
            return 0, False, f"Not enough stock for these product - {item['product_code']}"

        # getting the average price from all the pricelist
        market_price_query_set = product.supplier_maps.aggregate(Avg('price_maps__price_per_unit'))
        ...
        for supp_prod in supp_prod_list:
            ...
            pricing_engine = PricingEngine(supp_base_price, 20, prod_market_price, avg_past_sell_price)
            markup_price = pricing_engine.calculate_price()
            temp_shipping_price, _ = Fleet.calculate_price(total_weight, self.cust_obj.city, supp_prod.supplier.city)
    ...
    def get_shipping_price(self, prod_price_matched):
        shipping_detail = []
        total_shipping = 0
        sorted_prod_prices = sorted(prod_price_matched, key=lambda d: d['supplier_code'])
        for supplier_code, value in groupby(sorted_prod_prices, self.group_supplier):
            ...
            shipping_price, _ = Fleet.calculate_price(supp_weight, self.cust_obj.city, value_list[0]['supplier_city'])
    ...
    def get_best_price(self, chosen_supplier_code, total_price, prod_price_matched, prod_price_ambiguous, supp_checked=False):
            ...
            if not supp_checked:
                ...
                for price in prices:
                    ...
                    if price['temp_with_shipping'] < min_total_price:
                        best_price = price
                total_price += price['total_prod_price']
                prod_price_ambiguous.remove(prod)
                prod['prices'] = [best_price]
                prod_price_matched.append(self.prepare_matched_prod_data(prod))
                chosen_supplier_code.append(price['supplier'].code)
