...
def generate_application_no(string_length=6):
...
class Quote(SoftDeletionModel):
    ...
    customer = models.ForeignKey(
        Customer, on_delete=models.CASCADE, help_text="Foreign key of Customer", related_name="quotes"
    )
    quote_no = models.CharField(max_length=15, unique=True, null=False, default=generate_application_no)
    status = models.CharField(max_length=100, choices=STATUS_CHOICES, default=STATUS_INITIATED, db_index=True)
    expiry_date = models.DateTimeField(blank=True, null=True, help_text="Date of the expiry of quote if given")
    payable_amount = models.DecimalField(
        default=0.0, max_digits=30, decimal_places=4, help_text="Total Payable that customer need to pay"
    )
    price_breakdown = models.JSONField(null=False, default=dict, help_text="Breakdown of the total price")
...
class QuoteItem(SoftDeletionModel):
    ...
    quote = models.ForeignKey(
        Quote, on_delete=models.CASCADE, help_text="Foreign key of Quote", related_name="items"
    )
    product = models.ForeignKey(
        Product, on_delete=models.CASCADE, help_text="Foreign key of Product", related_name="quote_items"
    )
    supplier = models.ForeignKey(
        Supplier, on_delete=models.CASCADE, null=True, help_text="Foreign key of Supplier", related_name="quote_items"
    )
    remarks = models.TextField(null=True, blank=True, help_text='Remarks of the quote item')
    ...
    shipping_price = models.DecimalField(
        default=0.0, max_digits=25, decimal_places=4, help_text="Shipping price of the item"
    )
    objects = SoftDeletionManager()
    history = HistoricalRecords()
