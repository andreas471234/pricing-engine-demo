...
class Customer(SoftDeletionModel):
    code = models.CharField(
        ...
        unique=True,
        db_index=True,
        help_text="Customer unique code",
    )
    name = models.CharField(
        max_length=127, null=False, help_text="Name of the customer"
    )
    address = models.TextField(null=False, help_text="Address of Customer")
    city = models.CharField(
        max_length=127, null=False, help_text="City of Customer address"
    )
    state = models.CharField(
        max_length=127, null=False, help_text="State of Customer address"
    )
    ...
    def get_list(filters):
        ...
        cust_list = (
            Customer.objects.filter(**_filters).order_by("created_at").distinct()
        )
