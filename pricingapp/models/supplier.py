...
class Supplier(SoftDeletionModel):
    code = models.CharField(
        ...
        unique=True,
        db_index=True,
        help_text="Supplier unique code",
    )
    name = models.CharField(
        max_length=127, null=False, help_text="Name of the supplier"
    )
    address = models.TextField(null=False, help_text="Address of Supplier")
    city = models.CharField(
        max_length=127, null=False, help_text="City of Supplier address"
    )
    state = models.CharField(
        max_length=127, null=False, help_text="State of Supplier address"
    )
    ...
    def get_list(filters):
        ...
        supp_list = (
            Supplier.objects.filter(**_filters).order_by("created_at").distinct()
        )
...
class SupplierProduct(SoftDeletionModel):
    ...
    unit = models.ForeignKey(
        ProductUnit,
        on_delete=models.CASCADE,
        help_text="Foreign key of unit for the default stock",
    )
    objects = SoftDeletionManager()
...
class ProductPrice(SoftDeletionModel):
    supplier_map = models.ForeignKey(
        SupplierProduct,
        on_delete=models.CASCADE,
        help_text="Foreign key of supplier product map",
    ...
    price_per_unit = models.DecimalField(
        ...
        max_digits=25,
        decimal_places=4,
        help_text="Price per unit of product",
    )
    min_qty = models.FloatField(
        default=0,
        help_text="Minimal qty that they sell this unit in to get these prices",
    )
    unit = models.ForeignKey(
        ProductUnit, on_delete=models.CASCADE, help_text="Foreign key of unit"
    )
    objects = SoftDeletionManager()
